...
attach tree1 tree2
    | prio tree1 < prio tree2 = Node (prio tree1) (key tree1) (tree2 : (children tree1))
    | otherwise = Node (prio tree2) (key tree2) (tree1 : (children tree2))
...
insertTree tree trees = case trees of
    -- lista nula -> lista doar cu tree
    [] -> [tree]
    -- analog inc (0 : rest_of_number)
    (EmptyTree : tt) -> tree : tt
    -- analog inc (1 : rest_of_number)
    (t : tt) -> EmptyTree : (insertTree newTree tt)
        where newTree = attach tree $ head trees
...
-- mereu se returneaza un heap nou cu size incrementat si tree-ul nou cu proprietatile din parametri inserat la lista de trees a heap-ului anterior
insert prio key heap = BinomialHeap (size heap + 1) (insertTree newTree $ trees heap)
    where newTree = Node prio key []
...
findMin heap = pairIfNotNull nonEmptyTrees --functie auxuliara pentru pattern matching
    where
    nonEmptyTrees = [(Node p k children) | (Node p k children) <- trees heap]
    -- daca nu s-au gasit non-empty trees, return Nothing
    pairIfNotNull [] = Nothing
    -- altfel forteaza executia perechii cu Just
    pairIfNotNull nonNullList = Just (prio minimumTree, key minimumTree)
    -- returneaza tree-ul cu prioritate minima
    minimumTree = minimumBy (compare `on` prio) nonEmptyTrees
...
mergeTrees trees1 trees2 = check carry
    where
    -- check face pattern matching pentru a determina daca e nevoie sa adaug manual transportul imbinarii la rezultat
    check EmptyTree = partialSum
    check _ = partialSum ++ [carry]
    -- folosesc mapAccumL exact ca la adunarea a doua numere binare, in acumulator tin minte arborele de transport si in lista imi calculez rezultatul
    carry = fst $ mapAccumL (\acc (t1, t2) -> (accFunction acc t1 t2, mapFunction acc t1 t2)) EmptyTree zippedTrees
    partialSum = snd $ mapAccumL (\acc (t1, t2) -> (accFunction acc t1 t2, mapFunction acc t1 t2)) EmptyTree zippedTrees
    -- zipExtend pentru ca se poate ca listele de arbori sa aiba lungimi diferite
    zippedTrees = zipExtend EmptyTree EmptyTree trees1 trees2
    -- pattern matching pentru determinarea acumulatorului
    accFunction _ EmptyTree EmptyTree = EmptyTree
    accFunction EmptyTree _ EmptyTree = EmptyTree
    accFunction EmptyTree EmptyTree _ = EmptyTree
    accFunction EmptyTree t1 t2 = attach t1 t2
    accFunction acc EmptyTree t2 = attach acc t2
    accFunction acc t1 EmptyTree = attach acc t1
    accFunction acc t1 t2 = attach t1 t2
    -- pattern matching pentru construirea rezultatului (analogie cu suma numerelor binare: determinarea "bitului curent")
    mapFunction EmptyTree EmptyTree EmptyTree = EmptyTree
    mapFunction acc EmptyTree EmptyTree = acc
    mapFunction EmptyTree t1 EmptyTree = t1
    mapFunction EmptyTree EmptyTree t2 = t2
    mapFunction EmptyTree t1 t2 = EmptyTree
    mapFunction acc EmptyTree t2 = EmptyTree
    mapFunction acc t1 EmptyTree = EmptyTree
    mapFunction acc t1 t2 = acc
...
-}
isolate :: a -> [a] -> [(a, [a])]
isolate _ [] = [] -- pattern match pe [] pentru ca altfel nu functioneaza last pe lista goala
-- concatenez la final elementul fara placeholder pe ultima pozitie, deoarece functia din map l-ar inlocui si pe acela cu placeHolder
isolate placeHolder list = (map f (zip list [1..(len - 1)])) ++ [(last list, init list)]
    where
    f = (\(x, count) -> (x, g count))
    len = length list
    g count = (take (count - 1) list) ++ [placeHolder] ++ (drop count list)
...
removeMin (BinomialHeap 0 []) = emptyHeap
removeMin heap = BinomialHeap ((size heap) - 1) newTrees
    where
    -- lista initiala de arbori
    heapTrees = trees heap
    -- lista finala de arbori dupa eliminare
    newTrees = mergeTrees treesWithoutMin minOrphans
    -- lista de arbori non-vizi, pentru a selecta minimul
    nonEmptyTrees = [(Node p k children) | (Node p k children) <- trees heap]
    -- arborele cu prioritate minima, cuplat cu lista de arbori fara acesta
    (minTree, treesWithoutMin) = head $ filter filterMin $ isolate EmptyTree heapTrees
    filterMin (EmptyTree, _) = False
    filterMin (t, lt) = (t == (minimumBy (compare `on` prio) nonEmptyTrees))
    -- lista inversata de arbori copil ai arborelui cu cheie minima
    minOrphans = reverse $ children minTree
...
-- nod intern
printLevel space level (Node p k chldr) =  replicate (2 * level) space ++ show p ++ " (" ++ show k ++ ")\n" 
                                           ++ intercalate "\n" (map (printLevel space (level + 1)) chldr)

instance (Show p, Show k) => Show (BinomialTree p k) where
    show EmptyTree = "*"
    show tree@Node{} = printLevel ' ' 0 tree
